...
public class BookController {
    ...
    @GetMapping
    public ResponseEntity<ApiResponseDto<Page<BookEntity>>> getBooks( Pageable pageable) {
    ...
    @Operation(summary = "Listar todos os livros", description = "Retorna uma lista de todos os livros disponíveis no catálogo utilizando pelo Redis para cachê.",
            responses = {@ApiResponse(responseCode = "200", description = "Livros encontrados com sucesso",
                    ...
                    @ApiResponse(responseCode = "400", description = "Requisição inválida")})
    @GetMapping("all")
    public ResponseEntity<ApiResponseDto<List<BookEntity>>> getAllBooks() {
        List<BookEntity> books = bookService.getBooksAll();
        return ResponseEntity.ok(new ApiResponseDto<>(HttpStatus.OK.value(), "FOUND", books));
    }
